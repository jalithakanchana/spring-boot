...
public class SemanticGAPI {
    ...
    public void SemanticAnalysis(String[] args) throws Exception {
        if (args.length != 2) {
            System.err.println("Usage:");
            System.err.printf(
                    "\tjava %s \"command\" \"text to analyze\"\n", SemanticGAPI.class.getCanonicalName());
            System.exit(1);
        }
...
//            } else {
    ...
    public static void analyzeEntitiesFile(String gcsUri) throws Exception {
        ...
        try (LanguageServiceClient language = LanguageServiceClient.create()) {
            ...
            Document doc =
                    Document.newBuilder().setGcsContentUri(gcsUri).setType(Document.Type.PLAIN_TEXT).build();
            AnalyzeEntitiesRequest request =
                    AnalyzeEntitiesRequest.newBuilder()
                            .setDocument(doc)
                            .setEncodingType(EncodingType.UTF16)
